...
调整后，`dev` / `test` 仍保留 OpenAPI 能力，`prod` 默认关闭 OpenAPI JSON 与 Swagger UI，并且 Security 不再公开放行这些路径。补充回归测试后，也能证明即使携带合法 JWT，生产环境仍不会返回文档资源。
...
- 修改了什么
  ...
  - `OpenApiProductionSecurityTest` 补充已认证访问断言：管理员登录后携带合法 JWT 访问 `/v3/api-docs` 与 `/swagger-ui.html` 均返回统一 404。
...
- 关键设计原因
  ...
  - 已认证访问返回 404 的测试能区分两层安全含义：未登录请求被安全层挡住，已登录请求也因为 springdoc 关闭而拿不到文档资源。
...
## 5. 测试与验证
- 已执行：
  - `mvn -q -Dtest=OpenApiProductionSecurityTest test`
  - `mvn -q test`
  - `mvn -q -DskipTests package`
  - `git diff --check`
- 验证结果：
  - 本次补充的生产 OpenAPI 定向测试通过。
  - 定向测试通过。
  - 全量测试通过：54 个测试，0 failure，0 error，0 skipped。
  - 跳过测试打包通过。
  - diff 空白检查通过。
- 覆盖场景：
  ...
  - `prod` profile 下携带合法管理员 JWT 访问 `/v3/api-docs` 返回 404，错误码为 `COMMON-404`。
  - `prod` profile 下携带合法管理员 JWT 访问 `/swagger-ui.html` 返回 404，错误码为 `COMMON-404`。
...
- 对携带合法 JWT 的生产文档路径，当前依赖 springdoc 端点关闭来返回 404；如果未来显式开启 springdoc，需要同步复核这条测试预期。

## 7. 对后续版本的影响
- 对简历可用版的价值
  - 能展示“开发便利”和“生产安全”按 profile 隔离的工程意识。
  - 能解释 Swagger UI、OpenAPI JSON、Security 白名单之间的区别和组合风险。
  - 新增生产 profile 测试，让安全配置不只停留在 YAML 注释层面，并覆盖未认证与已认证两类访问路径。
- 对微服务 / 云原生演进的影响
