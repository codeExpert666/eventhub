 ...
 *
 * <p>这类测试对刚接触 Spring Boot 测试的同学来说，最容易困惑的一点是：
 * 它看起来像“单元测试”，但实际上更接近“轻量集成测试”。
 *
 * <p>原因在于这里并不是直接 new 一个 Controller 再手动调用方法，
...
class SystemControllerTest {
     ...
     * Spring 提供的 MVC 测试入口。
     *
     * <p>{@link MockMvc} 不会真的启动 Tomcat 之类的外部 Servlet 容器，
     * 但它会按 Spring MVC 的真实处理流程来执行请求，
     * 因此非常适合做控制器层和 Web 基础设施的测试。
     *
     * <p>你可以把它先理解成“内存里的 HTTP 客户端”：
     ...
     *
     * <p>这个测试重点不是业务复杂度，而是确认最基础的成功响应链路是否完整：
     ...
     *
     * <p>这个测试演示了控制器测试里最常见的 POST JSON 场景：
     * 先声明请求体内容类型，再传入 JSON 字符串，
     * 最后断言返回值中的业务字段是否和输入匹配。
     *
     * <p>因为这里的接口只是示例回显接口，所以断言重点是：
     ...
     *
     * <p>这里传入空字符串 {@code message}，目的是故意触发 {@code EchoRequest}
     * 上声明的校验注解。测试通过后说明：
     * 第一，请求已经被正确绑定到 DTO；
     ...
     *
     * <p>这个测试主要覆盖 {@link RequestIdFilter} 的安全边界：
     ...
     *
     * <p>这个接口通常不属于业务 Controller，但它对服务可观测性非常重要。
     ...
     *
     * <p>这说明 Swagger/OpenAPI 相关配置已经生效，
