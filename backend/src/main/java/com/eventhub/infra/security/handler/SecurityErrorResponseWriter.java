...
public class SecurityErrorResponseWriter {
     ...
     * 写出统一失败响应。
     *
     * @param request   HTTP 请求，用于读取 RequestIdFilter 写入的追踪标识
    ...
    public void write(
        ...
        // "timestamp": "响应生成时间"
        // }
        Object requestId = request.getAttribute(RequestContextConstants.REQUEST_ID_ATTRIBUTE);
        ApiResponse<Void> apiResponse = ApiResponse.failure(errorCode, message, null);
        if (requestId instanceof String value && !value.isBlank()) {
            // Security 异常发生在 Controller 之前，无法经过 ResponseBodyAdvice，
            // 因此由当前 HTTP 出口边界显式补充 requestId，保持响应头、响应体和日志一致。
            apiResponse.withRequestId(value);
        }
