 ...
 *
 * <p>
 * 这个配置类的核心目的，是把“普通 Java 接口形式的 Mapper”注册成 Spring 容器中的 Bean。
 ...
 * 以及 {@code resources/mapper/auth/UserMapper.xml} 这类 XML 文件中的 SQL statement
 * 完成数据库访问。
 * </p>
 *
 * <p>
 * 这个配置之所以会生效，是因为当前类位于 {@code com.eventhub} 根包之下。
 * Spring Boot 启动类会从根包开始做组件扫描，发现带有 {@link Configuration} 的类后，
 * 会把它当成配置类加载；随后 MyBatis-Spring 读取 {@link MapperScan} 的扫描规则，
 * 扫描符合条件的 Mapper 接口并注册对应代理 Bean。
 * </p>
 *
 * <p>
 * 注意：这个类没有显式声明 {@code @Bean} 方法是正常的。
 * 它的配置能力来自类上的注解，属于“声明式配置”；启动阶段框架会读取这些注解并完成 Bean 注册。
 * </p>
 */
/*
 * @Configuration:
 ...
 * UserMapper、leMapper 都位于 com.eventhub.modules.auth.mapper 包。
